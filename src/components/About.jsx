...
function AboutCard({ icon, title, text }) {
  return (
    <div
      className="about-card-deco p-8 transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(74,44,10,0.15)]"
    ...
    >
      ...
      <h3
        ...
        style={{ color: 'var(--red)', borderBottom: '1px solid var(--gold)' }}
      >
        {title}
      </h3>
...
function StageCard({ num, period, desc }) {
  return (
    ...
    >
      <div className="font-['Playfair_Display'] text-[1.8rem] font-bold" style={{ color: 'var(--gold)' }}>
        {num}
      </div>
      <div className="text-[0.9rem] italic mb-1" style={{ color: 'var(--red)' }}>{period}</div>
...
export default function About() {
  return (
    <section
      ...
      style={{
        background: '#F5EDD6',
        borderTop: '6px solid #4A2C0A',
        borderBottom: '6px solid #4A2C0A',
      }}
    >
      <div className="section-inner">
        ...
        <div className="grid grid-cols-2 gap-10 mt-8 max-sm:grid-cols-1">
          ...
          >
            <blockquote className="font-['IM_Fell_English'] italic text-[1.25rem] leading-[1.6]">
              „Într-un parteneriat adevărat, nevoile partenerului sunt tot atât de importante ca ale
              tale însuți. Cu nimic mai importante, cu nimic mai puțin importante. Exact la fel de
              importante."
            </blockquote>
            <cite
        ...
        <div
          ...
          style={{ border: '2px solid var(--gold)', background: '#4A2C0A' }}
        >
          <div style={{ color: 'var(--parchment)' }}>
            ...
            <span className="block mt-1 text-[0.85rem] italic" style={{ color: 'var(--gold-lt)' }}>
              Echipa „Făgurași" — clasa VIII-A
            </span>
          </div>

          <div className="w-px h-60px opacity-30" style={{ background: 'var(--gold)' }} />
          <div className="text-2rem opacity-50" style={{ color: 'var(--gold)' }}>♪ ⟺ ♪</div>
          <div className="w-px h-60px opacity-30" style={{ background: 'var(--gold)' }} />
